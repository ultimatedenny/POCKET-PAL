...
{
    public static class ClassSysUtils
    {
        static string ConnectionDB = System.Configuration.ConfigurationManager.ConnectionStrings["POCKETPALCON"].ToString();
        public static PP<string> NonQueryResults(this long NonQuery, string msg = null)
        {
            return new PP<string>()
            {
                Success = (NonQuery > 0) ? true : false,
        ...
        public static PP<string> OneRowdtScalarResult(this string NonQuery)
        {
            ...
            return new PP<string>
            {
                Success = sWordSplit[0].ToBoolean(),
        ...
        public static PP<string> OneRowdtToResult(this DataTable dt, string data = "")
        {
            return new PP<string>
            {
                ...
                Success = dt.Rows[0][0].ToBoolean(),
                data = data
            };
        }
        public static bool MoveFileToInternalFolderASP(string fileName, string FromDirectory, string ToDirectory)
        {
            string sourceFile = System.IO.Path.Combine(FromDirectory, fileName);
            string destFile = System.IO.Path.Combine(ToDirectory, fileName);
    ...
    {
        public string ComputerName { get; set; } = ClassSysUtils.DetermineCompName(HttpContext.Current.Request.UserHostName);
